		...
		<div>
			<ul className='flex flex-wrap justify-center  gap-1  cursor-pointer'>
				{pageNumbers.map((number, id) => (
					<li onClick={() => paginateFunc(number, id)} className={index === id ? 'bg-[#e58d27] px-5 py-3 border-2 border-white   ' : 'bg-[#777779]   px-5 py-3  '}>
						<a >{number}</a>
					</li>
				))}
			</ul>
      <div className='flex gap-2 mt-2 justify-center cursor-pointer'>
      <div onClick={prevPage } className='bg-[#777779]   px-5 py-3  '>Prev Page</div>
      <div onClick={nextPage } className='bg-[#777779]   px-5 py-3  '>Next Page</div>
      </div>
		</div>
	)
}
